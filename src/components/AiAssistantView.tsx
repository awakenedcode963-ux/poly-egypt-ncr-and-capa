...
export const AiAssistantView: React.FC<AiAssistantViewProps> = ({ masterData, logs }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Chat Messages */}
      <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/10 shadow-sm p-4 sm:p-6 space-y-4 min-h-[380px] max-h-[500px] overflow-y-auto">
        {messages.map((msg, idx) => (
          ...
          >
            <div className={`p-2 rounded-xl text-white shrink-0 ${
              msg.sender === 'user' ? 'bg-amber-500 text-white font-bold' : 'bg-slate-900'
            }`}>
            ...
            <div className={`p-4 rounded-2xl text-sm leading-relaxed max-w-[85%] whitespace-pre-line ${
              msg.sender === 'user'
                ...
                : 'bg-white/5 text-white border border-white/10/80 rounded-tl-none'
            }`}>
              {msg.text}
            </div>
          </div>
      ...
      >
        ...
        <button
          ...
          className="px-6 py-3 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-white font-bold text-sm rounded-xl shadow-md transition-all cursor-pointer flex items-center gap-2 shrink-0"
        >
