  ...
  return (
    <div className="fixed inset-0 bg-slate-950/70 z-50 flex items-center justify-center p-4 backdrop-blur-xs font-sans dir-rtl">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div id="printable-ncr-tag" className="p-6 overflow-y-auto space-y-6 print:p-8 bg-slate-50">
          {/* Logo & Company Name */}
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col items-center text-center">
            ...
            <div className="mt-2 pt-2 border-t border-slate-100 w-full flex items-center justify-between text-[10px] text-slate-500 font-bold">
          ...
          {/* QR Code Container */}
          <div className="bg-white p-5 rounded-2xl border-2 border-[#C4A052] shadow-md flex flex-col items-center justify-center text-center space-y-3">
          ...
          {capa ? (
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs space-y-2 text-xs">
              ...
              <div>
                <span className="text-slate-500 font-semibold block text-[10px]">الموضوع / العيب:</span>
                <span className="font-bold text-slate-900">{capa.subject}</span>
              </div>

              {capa.productName && (
                <div>
                  <span className="text-slate-500 font-semibold block text-[10px]">المنتج المتأثر:</span>
                  <span className="font-bold text-slate-800">{capa.productName}</span>
                </div>
              )}
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs text-xs space-y-1 text-slate-700 font-semibold text-center">
